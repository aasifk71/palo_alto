...
async function analyze() {
    // 1. Validation: Check for empty skills (Requirement 3: Basic Quality)
    if (skills.length === 0) {
        const input = document.getElementById('skillInput');
        // Visual feedback: briefly highlight the input box in red
        input.classList.add('border-red-400', 'bg-red-50');
        
        // Clear Error Message for the user
        alert("Please add at least one skill! Type a skill and press ENTER before clicking analyze.");
    ...
    // 2. Validation: Check if a role is selected
    if (!selectedRoleId) {
        alert("Please select a target role from the list first.");
        return;
    }
    
    const btn = document.getElementById('btn');
    const btnText = btn.querySelector('span');
    btnText.innerText = "Processing Roadmap...";
    btn.disabled = true;
    ...
    try {
        ...
        // 3. Handle API Errors (Requirement 3: Clear Error Messages)
        if (!res.ok) {
        ...
        // --- Rendering Logic (Remains the same as your current code) ---
        ...
        list.innerHTML = data.gaps.map(gap => `
            <div class="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                <div class="flex items-start justify-between mb-6">
                    ...
                    <div class="bg-emerald-50 text-emerald-600 text-[10px] font-black uppercase px-3 py-1 rounded-lg">High Priority</div>
                </div>
                    ...
                    ${gap.courses.map(c => `
                        <a href="${c.url}" target="_blank" class="group block">
                            <div class="relative rounded-2xl overflow-hidden mb-3 aspect-video bg-slate-100">
                                <img src="${c.thumbnail}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110 opacity-90 group-hover:opacity-100">
                            ...
                            <span class="font-bold text-sm text-slate-700 line-clamp-2 leading-tight group-hover:text-indigo-600 transition-colors">${c.title}</span>
                        </a>
                    `).join('')}
                </div>
            </div>
        ...
        // 4. Catch and show error to user (Requirement 3)
        console.error(e);
